...
<div class="panel panel-default">
    <div class="panel-body">
        ...
        <div class="table-responsive">
            <form action="<?=base_url('cursos/remove')?>" method="post" id="formSearch">
                <table class="table table-striped">
                    ...
                    <?php
                        foreach($find as $row){
                            ...
                            $tr .= '<td>'.$row->ci_curso.'</td>
                                        <td>'.$row->nm_curso.'</td>
                                        <td>'.$row->ds_eixo.'</td>';
                            if(isUpdate($transacao)) {
                                $tr .= '<td width="30" align="center">
                                                <button onclick="'.setLink(base_url('cursos/editar/'.$row->ci_curso)).'" type="button" class="btn btn-primary btn-rounded btn-xs" title="Editar" data-toggle="tooltip">
